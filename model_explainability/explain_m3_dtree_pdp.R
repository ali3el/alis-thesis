...
# ---- Generate PDPs ----
pdp_anxiety <- model_profile(explainer_m3_tree, variables = "anxiety_ever")
pdp_edu     <- model_profile(explainer_m3_tree, variables = "education_level")

# ---- Style Individual Plots ----
plot_anxiety_m3 <- plot(pdp_anxiety) +
  labs(
...
# ---- Combine & Save ----
combined_pdp_plot_m3 <- plot_anxiety_m3 / plot_edu_m3

ggsave(
  filename = here("model_explainability/results/pdp_combined_cleaned_m3.png"),
  plot = combined_pdp_plot_m3,
  width = 10, height = 10,
  dpi = 300
)

# Optional: Save RDA version
save(combined_pdp_plot_m3, file = here("model_explainability/results/pdp_combined_cleaned_m3.rda"))
