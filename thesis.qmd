...
Recent years have seen increased use of machine learning (ML) in mental health research, including efforts to predict depression from electronic health records, online behavior, or self-report surveys. For example, Nickson et al. (2023) conducted a systematic review of ML-based approaches using electronic health records to predict depression, highlighting a wide range of algorithms including logistic regression, support vector machines, and ensemble models, while also acknowledging the lack of subgroup-specific evaluation ([source](https://bmcmedinformdecismak.biomedcentral.com/articles/10.1186/s12911-023-02341-x?utm_source=chatgpt.com)). Similarly, Lee and Ham (2022) reviewed recent applications of machine learning in early depression diagnosis and noted the dominance of black-box models that prioritize performance over interpretability ([source](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC9441463/?utm_source=chatgpt.com)).

Even studies that aim to improve interpretability often focus on feature engineering or performance tuning, without accounting for identity-based variation in prediction quality. Furthermore, many predictive models operate as “black boxes,” offering high accuracy but limited insight into the mechanisms behind their predictions. This thesis builds on emerging work that uses interpretable models — such as decision trees — alongside explainability tools like SHAP values and partial dependence plots to provide transparent, actionable insights. The use of cross-validation, subgroup stratification, and feature engineering pipelines aligns with best practices in applied data science.
...
This work demonstrates the value of interpretable machine learning as a bridge between computational rigor and sociological inquiry, enabling both predictive insight and critical reflection on structural health disparities.


# Conclusion

This thesis set out to predict depression using interpretable machine learning models, with a particular focus on individuals whose identities often place them at the intersection of structural disadvantage, namely LGBTQ+ and migrant populations. Drawing on five years of National Health Interview Survey (NHIS) data, the project combined sociological insight with algorithmic modeling to assess how demographic, medical, and social factors contribute to depression risk.

Decision trees were chosen for their clarity and compatibility with explainability tools, enabling both strong predictive performance and transparent interpretation. Models trained using the Kitchen Sink and M3 feature sets achieved high F-measure and AUC scores, while SHAP values and local explanation plots made it possible to trace individual-level predictions. Stratifying the dataset across four subgroups ensured fairness across intersectional identities, with only minor variation in performance.
...
This project contributes to the growing body of work at the intersection of data science and social science by emphasizing transparency, subgroup fairness, and the integration of theory into applied modeling. Future work might explore additional model types, such as random forests or boosted trees, or extend this approach to other health outcomes and survey datasets. More broadly, this work serves as a case study in using machine learning not only to predict, but to reflect, helping surface the structural patterns that shape mental health across identity lines.

# Appendix

## Reproducibility & Data Access

All code used in this analysis is available in the accompanying GitHub repository (link below). Due to file size constraints, certain datasets and intermediate files have been uploaded separately to a publicly accessible Google Drive folder:

- Google Drive folder: [Insert link here]

To fully reproduce the results, users should download the files in that folder and place them in the appropriate directory as noted in the README or code comments. The main script files are structured for execution on both local machines and the Quest high-performance computing cluster.
